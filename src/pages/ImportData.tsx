...
const ImportData = () => {
  ...
  const handleFileUpload = async (
    ...
    if (file && selectedCategory) {
      ...
      const interval = setInterval(() => {
        progress = Math.min(progress + 10, 95); // Don't reach 100 until response
  ...
  const totalRecords = mockImports.reduce(
    (acc, imp) => acc + imp.recordsTotal,
    0
  );
  const successfulRecords = mockImports.reduce(
    (acc, imp) => acc + imp.recordsSuccess,
    0
  );
  const failedRecords = mockImports.reduce(
    (acc, imp) => acc + imp.recordsFailed,
    0
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <Card className="shadow-card border-0 bg-card/50 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>
            Select category and upload CSV/Excel file for bulk import
          </CardDescription>
        </CardHeader>
      ...
      {/* <Card className="shadow-card border-0 bg-card/50 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>
            Track previous data uploads and their status
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {mockImports.map((importItem) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
                  ...
                  <div className="text-center">
                    <div className="text-lg font-semibold text-primary">
                      {Math.round(
                        (importItem.recordsSuccess / importItem.recordsTotal) *
                          100
                      )}
                      %
                    </div>
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>
                    Uploaded by {importItem.uploadedBy} on{" "}
                    {importItem.uploadedAt}
                  </span>
